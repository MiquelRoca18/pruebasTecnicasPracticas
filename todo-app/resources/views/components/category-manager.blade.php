...
new class extends Component
{
    ...
    public function createCategory(): void
    {
        ...
        $this->dispatch('toast', message: __('app.category_created'), type: 'success');
    }
    ...
    public function deleteCategory(int $id): void
    {
        ...
        $this->dispatch('toast', message: __('app.category_deleted'), type: 'error');
    }
};
?>
...
<div class="space-y-4">
    ...
    <div class="bg-white rounded-2xl shadow-lg p-6" x-data="{ open: false }">
        ...
        <div x-show="open" x-transition @click.outside="open = false" class="mt-3 space-y-3">
            ...
            <div class="flex items-center gap-3">
                <label class="text-sm text-gray-500">{{ __('app.color') }}</label>
            ...
            <div class="flex justify-end gap-2">
                <button @click="open = false"
                    class="px-3 py-1.5 text-sm text-gray-500">{{ __('app.cancel') }}</button>
                <button
    ...
    <div class="bg-white rounded-2xl shadow-lg p-6">
        <h3 class="font-bold text-gray-700 mb-4">{{ __('app.category_list') }}</h3>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @empty
                <p class="text-sm text-gray-400">{{ __('app.no_categories') }}</p>
            @endforelse
        </div>
        ...
        @if ($totalCount > 4)
            ...
            >
                {{ $showAll ? __('app.see_less') : __('app.see_more') }}
            </button>
        @endif
    </div>
